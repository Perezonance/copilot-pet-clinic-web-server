  ...
  describe("updateAppointmentHandler()", () => {
    ...
    describe("error cases", () => {
      ...
      it("should return a 500 status code when the DB call fails", async () => {
        ...
        (AppointmentsDB.updateAppointment as jest.Mock).mockImplementationOnce(
          () => {
            throw new Error("DB Failure");
          }
        );
  ...
  describe("updateAppointmentStatusHandler()", () => {
    describe("golden path cases", () => {
      it("should return a 200 status code and the updated appointment", async () => {
        ...
        (
          AppointmentsDB.updateAppointmentStatus as jest.Mock
        ...
        expect(statusCode).toBe(200);
        expect(JSON.parse(payload)).toEqual(updatedAppointment);
        expect(AppointmentsDB.updateAppointmentStatus).toHaveBeenCalledTimes(1);
      });
    });
    ...
    describe("error cases", () => {
      ...
      it("should return a 403 status code when the vet ID in the headers does not match the vet ID of the appointment", async () => {
        const appointmentId = 40001;

        (
          AppointmentsDB.updateAppointmentStatus as jest.Mock
        ).mockResolvedValueOnce(mockDB[appointmentId]);
      ...
      it("should return a 400 status code when the status is invalid", async () => {
        const appointmentId = 40001;
        ...
        // Assert
        expect(statusCode).toBe(400);
        expect(AppointmentsDB.updateAppointmentStatus).toHaveBeenCalledTimes(0);
      });
